...
def rhcos_release_url(version, brew_arch="x86_64", private=False):
    """
...
def latest_rhcos_build_id(version, brew_arch="x86_64", private=False):
    return _latest_rhcos_build_id(version, brew_arch, private)


def _latest_rhcos_build_id(version, brew_arch="x86_64", private=False):
...
# this is hard to test with retries, so wrap testable method
@retry(reraise=True, stop=stop_after_attempt(10), wait=wait_fixed(3))
def rhcos_build_meta(build_id, version, brew_arch="x86_64", private=False, meta_type="meta"):
    return _rhcos_build_meta(build_id, version, brew_arch, private, meta_type)


def _rhcos_build_meta(build_id, version, brew_arch="x86_64", private=False, meta_type="meta"):
    """
...
def latest_machine_os_content(version, brew_arch="x86_64", private=False):
