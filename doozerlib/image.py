...
RUN yum install -y cov-sa csmock csmock-plugin-coverity csdiff
    ''')
            ...
            if not emit_path.exists():
                rc, out, err = exectools.cmd_gather(f'docker run --hostname=covscan --rm -v {str(cov_path)}:/cov:z {run_tag} cov-build --dir=/cov /{build_script_name}', realtime=True)
                if rc != 0:
            ...
            def run_docker_cov(cmd):
                return exectools.cmd_assert(f'docker run --hostname=covscan --rm -v {str(cov_path)}:/cov:z {run_tag} {cmd}', realtime=True)
